...
{
	int nLength;
	int nCRCType = 0, nCRCAddr = -1;
	int nInfo = 0;
	int32_t nStartAddr = -1, nEndAddr = -1;
	char sInFname[256] = {0}, sOutFname[256] = {0};
	...
	uint8_t *bufROM = malloc(MAX_ROM_SIZE);
	if (bufROM == NULL)
	{
	...
	uint16_t nSegment = 0, nExtSegment = 0;
	char str[256], s[16];
	uint16_t crc16 = 0;
	uint16_t chksum16 = 0;
	uint32_t chksum32 = 0;
	...
	while (fgets(str, 256, ifp))
	{
		...
		if (nType == 0)
		{
			int nAddress = (nSegment << 16) + (nExtSegment << 4) + nAddr;
			if (nStartAddr == -1)
			{
				nStartAddr = nAddress;
			}
			...
			if (((nAddress + nLength - nStartAddr) < MAX_ROM_SIZE) && (hexLastAddr < nAddress + nLength))
		...
		else if (nType == 2)
		{
			strncpy(s, &str[9], 4);
			s[4] = 0;
			nExtSegment = (uint16_t)strtol(s, NULL, 16);
		}
		else if (nType == 1)
		{
			if (nEndAddr == -1)
	...
	printf("  Info: start_address=%Xh, end_address=%Xh, crc_address=%Xh\n", nStartAddr, nEndAddr, crcAddr);
